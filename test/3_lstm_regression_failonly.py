...
def eval_pass(model, criterion, X, y, c, batch_size, device):
    """Run evaluation pass using pinned CPU memory with fast non-blocking transfer to GPU."""
    model.eval()
    ...
    with torch.no_grad():
        for i in range(math.ceil(n / batch_size)):
            # Fast slice + non-blocking transfer to GPU
...
def main():
    train_df, test_df, features = get_data()
    ...
    print("Training LSTM model with custom right-censored loss...")
    for epoch in range(epochs):
        ...
        for i in train_pbar:
            ...
            # Index on CPU, then copy slice asynchronously to GPU (non_blocking)
        ...
        val_loss, val_mse, val_mae, val_y_sum, val_y_sq_sum, val_uncensored = eval_pass(
            model, criterion, X_test, y_test, c_test, batch_size, device
        )

        # Single CPU transfer to minimize host-device synchronization overhead
        t_stats = torch.stack([
        ...
        print(f"  Val   -> Loss: {vl / len(X_test):.4f} | MSE: {v_mse:.4f} | RMSE: {v_rmse:.4f} | MAE: {v_mae:.4f} | R2: {v_r2:.4f}")
