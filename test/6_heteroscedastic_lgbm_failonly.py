...
def main():
    ...
    np.random.seed(42)
    val_sample_idx = np.random.choice(len(test_df), size=min(500000, len(test_df)), replace=False)
    val_sample_df = test_df.iloc[val_sample_idx]
    ...
    preds = gbm.predict(test_df[features])
    if len(preds.shape) == 1 or preds.ndim == 1:
        preds = preds.reshape(len(test_df), 2, order='F')
        
    mu = preds[:, 0]
    log_var = preds[:, 1]
    
    labels = test_df['log_RUL'].values
    censored = test_df['censored'].values
