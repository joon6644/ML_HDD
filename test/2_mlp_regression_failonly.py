...
def main():
    ...
    test_df['log_RUL'] = np.log1p(test_df['RUL'])
    ...
    y_test = torch.tensor(test_df['log_RUL'].values, dtype=torch.float32, device=device)
    c_test = torch.tensor(test_df['censored'].values, dtype=torch.float32, device=device)
    ...
    for epoch in range(epochs):
        ...
        num_val_batches = math.ceil(len(X_test) / batch_size)
        ...
        with torch.no_grad():
            for i in val_pbar:
                ...
                batch_c = c_test[i * batch_size : (i + 1) * batch_size]
        ...
        print(f"  Val   -> Loss: {(val_loss / len(X_test)).item():.4f} | MSE: {v_mse:.4f} | RMSE: {v_rmse:.4f} | MAE: {v_mae:.4f} | R2: {v_r2:.4f}")
