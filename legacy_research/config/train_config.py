...
# 서브셋 개수 동적 탐색 (기본값 5)
_SUBSET_DIR = PROJECT_ROOT / "data2" / "06_subset_generation" / f"seed_{SEED}"
if _SUBSET_DIR.is_dir():
    _N_SUBSETS = len(list(_SUBSET_DIR.glob("subset_*.parquet")))
    if _N_SUBSETS == 0:
        _N_SUBSETS = 5
else:
    _N_SUBSETS = 5
...
SAMPLER_KWARGS = dict(
    n_subsets   = _N_SUBSETS,       # 서브셋 개수 (앙상블 모델 수)
...
OPTUNA_DB_PATH  = str(_BASE / "notebooks2" / "optuna_study.db")  # SQLite 저장 경로 (절대 경로)
