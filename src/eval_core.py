...
# ════════════════════════════════════════════════════════════
# evaluate_row_level removed in compliance with STANDARDS.md
...
def run_final_evaluation_from_saved_model(
    ...
    *,
    ...
    cache_dir = getattr(cfg, "PREDICTIONS_DIR", cfg.MODEL_SAVE_DIR)
    cache_path = os.path.join(cache_dir, "test_probs.npy")
    use_cache = False
...
def run_evaluation(
    ...
    *,
    ...
    # ── 3. §8.1 행 단위 평가 생략 (STANDARDS.md 기준) ────────
    row_result = None
    ...
    min_alarms = getattr(cfg, "ROLLING_WINDOW_SIZE", 1)
    if min_alarms == 0:
        min_alarms = 1
    window_size = getattr(cfg, "ALARM_WINDOW", None)
    entity_result = ev.evaluate(
        df_test,
        y_test_prob,
        threshold,
        min_alarms=min_alarms,
...
def plot_disk_far_recall_curves(
    ...
    # [자동 저장 추가]
    save_dir = Path(__file__).resolve().parent.parent / "notebooks2" / "reports2" / "plots"
    save_dir.mkdir(parents=True, exist_ok=True)
    fn_curve = f"{prefix}_disk_far_recall_curve_labeled.png" if prefix else "disk_far_recall_curve_labeled.png"
    plt.savefig(save_dir / fn_curve, dpi=300)
    print(f"💾 Plot saved to: {save_dir / fn_curve}")
...
def plot_detailed_disk_analysis(
    ...
    plt.tight_layout()
    fn_cm = f"{prefix}_disk_confusion_matrix.png" if prefix else "disk_confusion_matrix.png"
    plt.savefig(save_dir / fn_cm, dpi=300)
    print(f"💾 Plot saved to: {save_dir / fn_cm}")
    plt.show()
    ...
    plt.tight_layout()
    fn_fa = f"{prefix}_false_alarm_days_distribution.png" if prefix else "false_alarm_days_distribution.png"
    plt.savefig(save_dir / fn_fa, dpi=300)
    print(f"💾 Plot saved to: {save_dir / fn_fa}")
    plt.show()
    ...
    plt.tight_layout()
    fn_lt = f"{prefix}_lead_time_distribution_zoomed.png" if prefix else "lead_time_distribution_zoomed.png"
    plt.savefig(save_dir / fn_lt, dpi=300)
    print(f"💾 Plot saved to: {save_dir / fn_lt}")
    plt.show()
    ...
    plt.tight_layout()
    fn_cdf = f"{prefix}_lead_time_cdf.png" if prefix else "lead_time_cdf.png"
    plt.savefig(save_dir / fn_cdf, dpi=300)
    print(f"💾 Plot saved to: {save_dir / fn_cdf}")
    plt.show()
...
def run_disk_level_threshold_tuning(cfg, show_plots: bool = True) -> dict:
    ...
    cache_dir = getattr(cfg, "PREDICTIONS_DIR", cfg.MODEL_SAVE_DIR)
    cache_path = os.path.join(cache_dir, "val_calib_probs.npy")
    if os.path.exists(cache_path):
...
def run_threshold_tuning_from_saved_model(
    ...
    *,
    ...
    df_grid = run_disk_level_grid_search(
        disks_data, thresholds, min_alarms_list, n_failed_disks, n_normal_disks,
        log_dir=getattr(cfg, "LOGS_DIR", cfg.MODEL_SAVE_DIR), window_size=window_size
    )

    # Save grid search results
    grid_csv_dir = getattr(cfg, "METRICS_DIR", cfg.MODEL_SAVE_DIR)
    grid_csv_path = os.path.join(grid_csv_dir, "disk_level_grid_search_results.csv")
    df_grid.to_csv(grid_csv_path, index=False, encoding='utf-8-sig')
    ...
    df_best_disk = pd.DataFrame(best_disk_records)
    best_disk_csv_dir = getattr(cfg, "METRICS_DIR", cfg.MODEL_SAVE_DIR)
    best_disk_csv_path = os.path.join(best_disk_csv_dir, "best_disk_lead_time_distribution_calib.csv")
    df_best_disk.to_csv(best_disk_csv_path, index=False, encoding='utf-8-sig')
