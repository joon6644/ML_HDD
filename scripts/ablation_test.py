...
if __name__ == "__main__":
    ...
    # delta 컬럼 추가 (baseline 대비)
    baseline_cv   = df_out.loc[0, "CV PR-AUC"]
    baseline_test = df_out.loc[0, "test PR-AUC"]
    df_out["Δ CV PR-AUC"]   = (df_out["CV PR-AUC"]   - baseline_cv).round(4)
    df_out["Δ test PR-AUC"] = (df_out["test PR-AUC"] - baseline_test).round(4)
